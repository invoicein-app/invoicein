// invoiceku/app/invoice/new/page.tsx  (FULL REPLACE)
"use client";
...
type Item = {
  ...
  // numeric values for calculation & payload
  qty: number;
  price: number;

  // text values for input (avoid leading 0 issue + allow thousand separators)
  qtyText: string;
  priceText: string;
};
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        // percent-only (disimpan di kolom discount_value & tax_value)
        discount_value: clampPercent(num(discountPercent)),
  ...
  return (
    <div style={{ padding: 18, maxWidth: 980, margin: "0 auto" }}>
      ...
      >
        <div>
          ...
          <p style={{ marginTop: 6, color: "#666" }}>
            Simpan dulu, nanti bisa view & download PDF
          </p>
        </div>
      ...
      >
        ...
        <div style={card()}>
          <h3 style={{ margin: 0 }}>Diskon & Pajak (Fix %)</h3>
          <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
            <label style={label()}>
              Diskon (%)
              <input
                ...
                onChange={(e) =>
                  setDiscountPercent(clampPercent(digitsToNumber(e.target.value)))
                }
                placeholder="0–100"
                style={input()}
              />
            ...
            <label style={label()}>
              ...
              <input
                ...
                onChange={(e) =>
                  setTaxPercent(clampPercent(digitsToNumber(e.target.value)))
                }
                placeholder="0–100"
                style={input()}
              />
            ...
            <div style={{ marginTop: 6, borderTop: "1px solid #eee", paddingTop: 10 }}>
              ...
              <Row k={`Diskon (${calc.discPct}%)`} v={rupiah(calc.disc)} />
              ...
              {items.map((it, i) => (
                <tr key={i}>
                  <td style={td()}>
                    ...
                    {loadingProd ? (
                      <small style={{ color: "#666" }}>Loading barang...</small>
                    ) : null}
                  </td>
                  ...
                  <td style={td()}>
                    <input
                      value={it.name}
                      onChange={(e) =>
                        setItem(i, { name: e.target.value, product_id: "" })
                      }
